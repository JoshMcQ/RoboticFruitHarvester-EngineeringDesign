#!/usr/bin/env python3
"""
simple_gui_picker.py
Simple Tkinter GUI for YOLO11 pick & place with live camera feed.

Usage:
    python simple_gui_picker.py
"""
...
class PickerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("YOLO11 Pick & Place")
    ...
    def create_widgets(self):
        ...
        # Force threshold
        ttk.Label(control_frame, text="Force:").pack(side=tk.LEFT, padx=(20, 5))
        self.force_var = tk.DoubleVar(value=FORCE_THRESHOLD)
        force_spin = ttk.Spinbox(control_frame, from_=10, to=500, increment=10, textvariable=self.force_var, width=8)
        force_spin.pack(side=tk.LEFT)
        ...
        self.det_text = tk.Text(det_frame, height=8, width=35, font=("Consolas", 9))
        self.det_text.pack()
        ...
        self.staged_text = tk.Text(staged_frame, height=6, width=35, font=("Consolas", 9))
        ...
        self.log_text = tk.Text(log_frame, width=35, font=("Consolas", 9))
                ...
                if r.boxes is not None and len(r.boxes):
                    ...
                    if not df_fruit.empty:
                        ...
                        if depth_m is not None and np.isfinite(depth_m):
                            ...
                            det_info = f"Detection: {best_detection['name']}\n"
    ...
    def stage_pick(self):
        if hasattr(self, 'latest_detection') and self.latest_detection:
            ...
            self.log(f"📌 Staged: {sp['name']} at ({sp['xr']:.1f}, {sp['yr']:.1f}, {sp['zr']:.1f})")
        else:
    ...
    def _execute_pick_thread(self):
        try:
            ...
            smooth_close_with_force(self.arm, self.ser, self.force_var.get())
            time.sleep(0.2)
